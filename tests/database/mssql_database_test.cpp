...
namespace {
    ...
    std::optional<MSSQLConfig> loadMSSQLConfigFromEnv() {
        MSSQLConfig cfg;
        const char* hostEnv = std::getenv("SQLEDITOR_TEST_MSSQL_HOST");
        const char* portEnv = std::getenv("SQLEDITOR_TEST_MSSQL_PORT");
        const char* databaseEnv = std::getenv("SQLEDITOR_TEST_MSSQL_DB");
        const char* userEnv = std::getenv("SQLEDITOR_TEST_MSSQL_USER");
        const char* passwordEnv = std::getenv("SQLEDITOR_TEST_MSSQL_PASSWORD");
        const char* nameEnv = std::getenv("SQLEDITOR_TEST_MSSQL_NAME");
...
protected:
    void SetUp() override {
        ...
        ASSERT_TRUE(cfgOpt.has_value())
            ...
            << "Docker test databases automatically, or set SQLEDITOR_TEST_MSSQL_HOST / "
            << "SQLEDITOR_TEST_MSSQL_DB / SQLEDITOR_TEST_MSSQL_USER (optional PORT/PASSWORD/NAME).";
        ...
        tableName = TestHelpers::makeUniqueIdentifier("sqleditor_mssql_test_");
        cleanup();
    }
...
TEST_F(MSSQLDatabaseIntegrationTest, DropCurrentlyConnectedDatabaseSwitchesToMasterDatabase) {
    ...
    const std::string tempDb = TestHelpers::makeUniqueIdentifier("sqleditor_mssql_drop_active_");
