...
namespace {
    ...
    std::optional<OracleConfig> loadOracleConfigFromEnv() {
        OracleConfig cfg;
        const char* hostEnv = std::getenv("SQLEDITOR_TEST_ORACLE_HOST");
        const char* portEnv = std::getenv("SQLEDITOR_TEST_ORACLE_PORT");
        const char* databaseEnv = std::getenv("SQLEDITOR_TEST_ORACLE_DB");
        const char* userEnv = std::getenv("SQLEDITOR_TEST_ORACLE_USER");
        const char* passwordEnv = std::getenv("SQLEDITOR_TEST_ORACLE_PASSWORD");
        const char* nameEnv = std::getenv("SQLEDITOR_TEST_ORACLE_NAME");
    ...
    bool installerDownloadTestsEnabled() {
        const char* enabledEnv = std::getenv("SQLEDITOR_TEST_ORACLE_INSTALLER_DOWNLOAD");
...
TEST_F(OracleClientInstallerTest, DownloadAndInstallOracleClient) {
    if (!installerDownloadTestsEnabled()) {
        GTEST_SKIP() << "Set SQLEDITOR_TEST_ORACLE_INSTALLER_DOWNLOAD=1 to run download-based tests";
    }
...
protected:
    void SetUp() override {
        ...
        ASSERT_TRUE(cfgOpt.has_value())
            << "Missing Oracle test configuration. Set SQLEDITOR_TEST_ORACLE_HOST / "
            << "SQLEDITOR_TEST_ORACLE_DB / SQLEDITOR_TEST_ORACLE_USER (optional PORT/PASSWORD/NAME).";
