...
namespace {
    ...
    std::optional<PostgresConfig> loadPostgresConfigFromEnv() {
        PostgresConfig cfg;
        const char* hostEnv = std::getenv("SQLEDITOR_TEST_PG_HOST");
        const char* portEnv = std::getenv("SQLEDITOR_TEST_PG_PORT");
        const char* databaseEnv = std::getenv("SQLEDITOR_TEST_PG_DB");
        const char* userEnv = std::getenv("SQLEDITOR_TEST_PG_USER");
        const char* passwordEnv = std::getenv("SQLEDITOR_TEST_PG_PASSWORD");
        const char* nameEnv = std::getenv("SQLEDITOR_TEST_PG_NAME");
...
protected:
    void SetUp() override {
        ...
        ASSERT_TRUE(cfgOpt.has_value())
            ...
            << "SQLEDITOR_TEST_PG_HOST / SQLEDITOR_TEST_PG_DB / SQLEDITOR_TEST_PG_USER "
        ...
        tableName = TestHelpers::makeUniqueIdentifier("sqleditor_pg_test_");
        cleanup();
    }
...
TEST_F(PostgresDatabaseIntegrationTest, DropCurrentlyConnectedDatabaseSwitchesToPostgresDatabase) {
    ...
    const std::string tempDb = TestHelpers::makeUniqueIdentifier("sqleditor_pg_drop_active_");
...
TEST_F(PostgresDatabaseIntegrationTest, CreateDatabaseWithOptionsCreatesDatabaseAndComment) {
    ...
    const std::string tempDb = TestHelpers::makeUniqueIdentifier("sqleditor_pg_create_opts_");
    ...
    options.comment = "sqleditor option test's comment";
...
protected:
    void SetUp() override {
        ...
        schemaName = TestHelpers::makeUniqueIdentifier("sqleditor_pg_schema_");
        viewName = TestHelpers::makeUniqueIdentifier("sqleditor_pg_view_");
    }
...
TEST_F(PostgresSchemaNodeDDLTest, RoutinesLoadAsyncFindsRoutines) {
    const std::string funcName = TestHelpers::makeUniqueIdentifier("sqleditor_pg_func_");
