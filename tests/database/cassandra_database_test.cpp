...
namespace {
    ...
    std::optional<CassandraConfig> loadCassandraConfigFromEnv() {
        CassandraConfig cfg;
        const char* hostEnv = std::getenv("SQLEDITOR_TEST_CASSANDRA_HOST");
        const char* portEnv = std::getenv("SQLEDITOR_TEST_CASSANDRA_PORT");
        const char* userEnv = std::getenv("SQLEDITOR_TEST_CASSANDRA_USER");
        const char* passwordEnv = std::getenv("SQLEDITOR_TEST_CASSANDRA_PASSWORD");
        const char* nameEnv = std::getenv("SQLEDITOR_TEST_CASSANDRA_NAME");
...
protected:
    void SetUp() override {
        ...
        if (!cfgOpt.has_value()) {
            GTEST_SKIP() << "Skipping Cassandra tests: SQLEDITOR_TEST_CASSANDRA_HOST not set. "
        ...
        keyspace_ = TestHelpers::makeUniqueIdentifier("sqleditor_cass_test_");
        cleanup();
    }
