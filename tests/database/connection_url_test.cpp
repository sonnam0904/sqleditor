...
TEST(ConnectionUrlTest, MongoReplicaSetSeedList) {
    const std::string url =
        "mongodb://10.5.18.241:27017,10.3.29.198:27017,172.16.2.59:27017/livechat?"
        "replicaSet=bizflyLivechat01";
    auto r = parseConnectionUrl(url);
    ASSERT_TRUE(r.ok) << r.error;
    EXPECT_EQ(r.info.type, DatabaseType::MONGODB);
    EXPECT_EQ(r.info.host, "10.5.18.241:27017,10.3.29.198:27017,172.16.2.59:27017");
    EXPECT_EQ(r.info.port, 27017);
    EXPECT_EQ(r.info.database, "livechat");
    EXPECT_EQ(r.info.mongoOptions, "replicaSet=bizflyLivechat01");
    EXPECT_EQ(r.info.buildConnectionString(), url);
}

TEST(ConnectionUrlTest, MongoReplicaSetRoundTrip) {
    DatabaseConnectionInfo info;
    info.type = DatabaseType::MONGODB;
    info.host = "10.5.18.241:27017,10.3.29.198:27017";
    info.port = 27017;
    info.database = "livechat";
    info.mongoOptions = "replicaSet=bizflyLivechat01";

    const std::string url = buildConnectionUrl(info);
    EXPECT_EQ(url,
              "mongodb://10.5.18.241:27017,10.3.29.198:27017/livechat?replicaSet=bizflyLivechat01");

    auto r = parseConnectionUrl(url);
    ASSERT_TRUE(r.ok) << r.error;
    EXPECT_EQ(r.info.host, info.host);
    EXPECT_EQ(r.info.database, info.database);
    EXPECT_EQ(r.info.mongoOptions, info.mongoOptions);
}
