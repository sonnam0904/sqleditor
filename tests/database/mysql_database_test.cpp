...
namespace {
    ...
    std::optional<MySQLConfig> loadMySQLConfigFromEnv() {
        MySQLConfig cfg;
        const char* hostEnv = std::getenv("SQLEDITOR_TEST_MYSQL_HOST");
        const char* portEnv = std::getenv("SQLEDITOR_TEST_MYSQL_PORT");
        const char* databaseEnv = std::getenv("SQLEDITOR_TEST_MYSQL_DB");
        const char* userEnv = std::getenv("SQLEDITOR_TEST_MYSQL_USER");
        const char* passwordEnv = std::getenv("SQLEDITOR_TEST_MYSQL_PASSWORD");
        const char* nameEnv = std::getenv("SQLEDITOR_TEST_MYSQL_NAME");
...
protected:
    void SetUp() override {
        ...
        ASSERT_TRUE(cfgOpt.has_value())
            ...
            << "Docker test databases automatically, or set SQLEDITOR_TEST_MYSQL_HOST / "
            << "SQLEDITOR_TEST_MYSQL_DB / SQLEDITOR_TEST_MYSQL_USER (optional PORT/PASSWORD/NAME).";
        ...
        tableName = TestHelpers::makeUniqueIdentifier("sqleditor_mysql_test_");
        cleanup();
    }
...
TEST_F(MySQLDatabaseIntegrationTest, DropCurrentlyConnectedDatabaseSwitchesToMysqlDatabase) {
    ...
    const std::string tempDb = TestHelpers::makeUniqueIdentifier("sqleditor_mysql_drop_active_");
...
TEST_F(MySQLDatabaseIntegrationTest,
    ...
    const std::string tempDb = TestHelpers::makeUniqueIdentifier("sqleditor_mysql_create_opts_");
