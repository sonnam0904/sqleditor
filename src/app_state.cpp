...
namespace {
    ...
    bool parseConnectionRow(sqlite3_stmt* stmt, SavedConnection& conn) {
        ...
        } else if (typeStr == "cassandra") {
            conn.connectionInfo.type = DatabaseType::CASSANDRA;
        } else {
        ...
        std::string mongoOptionsStr = columnText(stmt, 22);
        conn.connectionInfo.mongoOptions =
            (mongoOptionsStr == "NULL") ? "" : mongoOptionsStr;
...
AppState::AppState() {
    ...
    if (home) {
        dbPath_ = fs::path(home) / ".sqleditor" / "connections.db";
    } else {
...
bool AppState::createTables() {
    ...
    ensureColumnExists("mongo_options",
                       "ALTER TABLE saved_connections ADD COLUMN mongo_options TEXT;");
...
int AppState::saveConnection(const SavedConnection& connection) const {
    const std::string sql = R"(
        ...
        (name, type, host, port, database_name, username, password, path, salt, last_used, workspace_id,
         ...
         ssl_ca_cert_path, mongo_options)
    ...
    sqlite3_bind_text(stmt.get(), 20, connection.connectionInfo.sslCACertPath.c_str(), -1,
                      SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt.get(), 21, connection.connectionInfo.mongoOptions.c_str(), -1,
                      SQLITE_TRANSIENT);
...
std::vector<SavedConnection> AppState::getSavedConnections() const {
    ...
    const std::string sql = R"(
        SELECT id, name, type, host, port, database_name, username, password, path, salt, last_used,
               ...
               COALESCE(mongo_options, '') as mongo_options
