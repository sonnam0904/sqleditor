...
struct DatabaseConnectionInfo {
    ...
    std::string mongoOptions;          // MongoDB URI query params (e.g. replicaSet=name)
    SSHConfig ssh;
    ...
    [[nodiscard]] std::string buildConnectionString(const std::string& dbName = "") const {
        ...
        case DatabaseType::MONGODB: {
            ...
            const int effectivePort = port > 0 ? port : 27017;
            if (host.find(',') != std::string::npos) {
                connStr += host;
            } else {
                connStr += host + ":" + std::to_string(effectivePort);
            }
            ...
            std::string query = mongoOptions;
            auto appendQuery = [&](const std::string& part) {
                if (query.empty()) {
                    query = part;
                } else {
                    query += "&" + part;
